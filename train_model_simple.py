#!/usr/bin/env python3
"""
Brain Tumor Detection CNN Training - Simple Version
==================================================

This version is optimized for very small datasets and testing purposes.
"""
...
# Configuration
IMG_SIZE = 128
BATCH_SIZE = 1  # Very small batch size for tiny dataset
EPOCHS = 30
LEARNING_RATE = 0.001
...
def build_simple_cnn():
    ...
    model = Sequential([
        ...
        Conv2D(8, (3, 3), activation='relu', input_shape=(IMG_SIZE, IMG_SIZE, 1)),
        BatchNormalization(),
        MaxPooling2D(2, 2),
        Dropout(0.25),
...
def main():
    ...
    print("🧠 BRAIN TUMOR DETECTION CNN TRAINING (Simple Version)")
    print("="*60)
    ...
    if len(X) < 20:  # Very small dataset
        print("⚠️  Very small dataset detected. Using all data for training.")
        print("⚠️  This is for testing purposes only. For production, use a larger dataset.")
        
        # Use all data for training, create a minimal validation set
        if len(X) >= 4:
            # Use 1 sample for validation if we have at least 4 samples
            ...
            X_train, y_train = X, y
            X_val, y_val = X[:1], y[:1]  # Use first sample as validation
        
        X_test = X_val  # Use validation as test set
    else:
